(function (root, factory) {
  ...
  function balanceText(elements) {
    forEach(getElementsList(elements), (el) => {
      ...
      const spaceWidth =
        oldWS === "pre-wrap" ? 0 : getSpaceWidth(el, nowrapHeight);
      if (
        containerWidth > 0 &&
        ...
        while (remLines > 1) {
          ...
          if (le.index === 0) {
            ...
            splitIndex =
              Math.abs(desiredWidth - le.width) <
              Math.abs(ge.width - desiredWidth)
                ? le.index
                : ge.index;
          }
